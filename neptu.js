...
function weton(x){
 ...
 var LakuneGeni = [15,'Lakune Geni','"Hasil Lakune Geni, Orang yang memiliki neptu 15 adalah orang yang lahir pada Rabu (7) Kliwon (8), Kamis(8) Pon (7) dan juga Jumat (6) Pahing (9). Watak dan kepribadian orang dengan neptu 15 adalah orang yang pendendam dan memiliki amarah yang besar. Ia tidak suka diperintah karena hatinya tegas. Dalam hidupnya ia akan sering berselisih dengan orang orang yang berada di dekatnya."']


 var LakuneBumi = [16,'Lakune Bumi','"Hasil Lakune Bumi, Orang yang memiliki neptu 16 adalah orang yang lahir pada Kamis(8) Kliwon (8), Sabtu (9) Pon (7) dan juga Rabu (7) Pahing (9). Watak dan kepribadian orang dengan neptu 16 adalah orang yang mudah diatur dan suka mengayomi. Ia adalah sosok yang cocok untuk menjadi seorang pemimpin yang baik. Sopan santun, pemaaf dan tidak mudah marah membuat dia memiliki banyak teman. Tetapi hati hati jika sampai ia marah maka akan mengerikan sekali."']


 var LakuneGunung = [17,'Lakune Gunung','"Hasil Lakune Gunung, Orang yang memiliki neptu 17 adalah orang yang lahir pada Sabtu (9) Kliwon (8) atau Kamis(8) Pahing (9). Watak dan kepribadian orang dengan neptu 17 adalah orang yang moody, pendiam, lambat dan terlalu baik. Apabila seseorang bisa mengambil hatinya maka ia akan mudah diatur."']


 var LakuneParipurna = [18,'Lakune Paripurna','"Hasil Lakune Paripurna, Orang yang memiliki neptu 18 adalah orang yang lahir pada Sabtu (9) Pahing (9). Watak dan kepribadian orang dengan neptu 18 adalah orang yang suka akan kekuasaan, setiap permintaannya harus dipenuhi. Ia juga jadi sosok yang mudah panas tetapi mudah mengalah jika amarahnya redah."']




    if(PenditoKangLelaku.some(item => item == x)){
        return [PenditoKangLelaku[8], PenditoKangLelaku[9], "FFC4C8"];
    }else if(LakuneGeni.some(item => item == x)){
        return [LakuneGeni[4], LakuneGeni[5], "C6FFC2"];
    }else if(LakuneAngin.some(item => item == x)){
        return [LakuneAngin[4], LakuneAngin[5], "C6FFC2"];
    }else if(PenditoMbangunTeki.some(item => item == x)){
        return [PenditoMbangunTeki[4], PenditoMbangunTeki[5], "E5E4C7"];
    }else if(LakuneSetan.some(item => item == x)){
        return [LakuneSetan[4], LakuneSetan[5], "E5E4C7"];
    }else if(LakuneKembang.some(item => item == x)){
        return [LakuneKembang[4], LakuneKembang[5], "FFC4C8"];
    }else if(LakuneLintang.some(item => item == x)){
        return [LakuneLintang[4], LakuneLintang[5], "FFC4C8"];
    }else if(LakuneMbulan.some(item => item == x)){
        return [LakuneMbulan[4], LakuneMbulan[5], "C6FFC2"];
    }else if(LakuneGeni.some(item => item == x)){
        return [LakuneGeni[4], LakuneGeni[5], "C6FFC2"];
    }else if(LakuneBumi.some(item => item == x)){
        return [LakuneBumi[4], LakuneBumi[5], "C6FFC2"];
    }else if(LakuneGunung.some(item => item == x)){
        return [LakuneGunung[4], LakuneGunung[5], "C6FFC2"];
    }else if(LakuneParipurna.some(item => item == x)){
        return [LakuneParipurna[4], LakuneParipurna[5], "C6FFC2"];
    }
}
